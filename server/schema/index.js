    ...
    subscription: new GraphQLObjectType({
        ...
        fields: {
            imageUpload: {
                ...
                subscribe: (_, { userId }, { redis }) => {
                    if (
                        redis.sub.subscription_set[
                            `subscribe_${userId}-${IMAGE_UPLOAD_CHANNEL}`
                        ]
                    ) {
                        pubsub.unsubscribe(IMAGE_UPLOAD_CHANNEL)
                    } else {
                        redis.sub.subscribe(`${userId}-${IMAGE_UPLOAD_CHANNEL}`)
                        redis.sub.on('message', function(channel, message) {
                            pubsub.publish(IMAGE_UPLOAD_CHANNEL, {
                                [IMAGE_UPLOAD_CHANNEL]: message
                            })
                        })
                    }

                    return pubsub.asyncIterator(IMAGE_UPLOAD_CHANNEL)
                }
            },
            conversation: {
                type: MessageType,
